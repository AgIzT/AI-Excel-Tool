...
| 类型 | 技术或依赖 | 用途 |
| --- | --- | --- |
| 桌面界面 | PySide6 | 主窗口、页面路由、文件拖拽、进度条、日志、复核弹窗 |
...
`requirements.txt` 仅 5 项：`PySide6`、`pandas`、`xlrd`、`openpyxl`、`openai`。重构时已移除 `requests`、`zhipuai`、`Pillow`。
...
├─ theme.py                    # 配色常量 + build_stylesheet() 生成 QSS
├─ widgets.py                  # 自定义控件 FileDropListWidget（拖拽落入文件）
...
│  └─ 多规格商品模板.xls        # 内置模板 2
├─ requirements.txt            # Python 运行依赖（5 项）
├─ README.md                   # 项目简介和快速上手
├─ LICENSE                     # MIT License
...
- 表现层：`main_app.py`（窗口外壳）+ `doc_page.py` / `settings_page.py`（页面 Mixin）+ `widgets.py` / `theme.py`。
...
- Mixin 自身不写 `__init__`，`super().__init__()` 沿 MRO 抵达 `QMainWindow`。
- 各页面的构建/事件方法分散在对应 Mixin 文件里，但 `self` 始终是同一个主窗口实例，共享 `self.settings`、`self._append_log` 等状态与方法。
- `main_app.py` 只保留外壳：窗口、侧边栏、路由、日志抽屉、样式、Qt 加速等。
...
- `main()`：配置 Qt OpenGL 渲染、创建 `QApplication`、设置 Fusion 风格、展示 `MainWindow`。
...
- 维护已选文件、输出文件、当前路由、后台任务状态、日志缓存、图片预览缓存。
- 构建侧边栏、顶部栏、页面容器（`QStackedWidget`）、日志抽屉。
- 按需构建并切换页面（首次访问某路由时才构建对应页面）。

页面路由包括：

| 页面 | 构建函数 | 来源 | 功能 |
| --- | --- | --- | --- |
| 单据处理 | `_build_doc_page()` | DocPageMixin | 文件导入、图片预览、模板选择、手写体识别、合并输出、开始识别 |
| 设置 | `_build_setting_page()` | SettingsPageMixin | 选择接口预设，填写 base_url / 模型 / API Key 并保存 |
...
- 多文件时用下拉框（`QComboBox` + `QStackedWidget`）在文件之间切换；单文件时隐藏切换条。
- 每个文件一页：可编辑的「供应商 / 日期」输入框 + 一张 `QTableWidget`（列=模板表头，行=识别记录）。
- 表格支持改单元格、「＋ 增加行」「✕ 删除选中行」。
- `get_edited_extraction()` 把表格与供应商/日期回读为与 `extract_batch` 同构的 `extraction`；`_read_table()` 会丢弃整行全空的记录。
- 「✅ 确认导出」前若总记录数为 0 会拦截提示；「取消」则不导出。
...
`build_log.txt` 是一次 PyInstaller 打包日志（PyInstaller 6.19.0，入口 `main_app.py`，含 `openpyxl`/`xlrd`/`pandas` 等 hidden imports）。当前仓库未提交 `.spec`，`.gitignore` 忽略 `*.spec`、`build/`、`dist/`。打包时注意把 `templates/` 与 `prompts/` 一并纳入数据文件。
...
- `build_log.txt` 是历史打包日志，不是可重复构建脚本；规范交付建议补 PyInstaller spec 或打包脚本，并确保打包包含 `templates/` 与 `prompts/`。
